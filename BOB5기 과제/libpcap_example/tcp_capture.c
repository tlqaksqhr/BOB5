...
int main(int argc,char **argv)
{
	...
	char filter_exp[] = "port 80"; // 패킷 필터링 옵션.
...
void process_packet(u_char *args,const struct pcap_pkthdr *header,const u_char *packet)
{
	int i,j,len,datalen,tcp_hdrlen;
	u_char *data;
	...
	tcp_hdrlen = TH_OFF(sniff_tcp)*4;
	data = (u_char *)(packet + sizeof(HDR_ETHERNET) + len + tcp_hdrlen);
	datalen = ntohs(sniff_ip->len) - len - tcp_hdrlen;
